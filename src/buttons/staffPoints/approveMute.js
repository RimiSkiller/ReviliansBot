...
module.exports = {
	...
	callback: async (client, interaction) => {
		...
		data.points++;
		await data.save();
		client.channels.cache.get(mainlog).send({ embeds: [new EmbedBuilder().setDescription(`- **Added a point for staff <@${user.id}> for registering a proof.**`).setColor(0x00ff00)] });
